...
- **Prochaine tâche (à la reprise)** :
  1. **Vérifier d'autres amulettes/objets uniques "Replica"** du même genre que "Réplique d'Envol de Crocdragon" (celle qui a révélé le bug de l'annotation de nom de gemme, voir ci-dessus) — ces objets uniques ont souvent plusieurs modificateurs différents les uns des autres, il faut vérifier au cas par cas si les AUTRES modificateurs (pas seulement celui de niveau de gemme déjà corrigé) passent bien en mode avancé. Demandé par Arnaud le 2026-07-22, à faire à la prochaine session.
