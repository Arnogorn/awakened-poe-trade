...
- **Vérifié le 2026-07-23** : d'autres objets uniques "Replica" testés par Arnaud en jeu, tous les modificateurs (pas seulement celui de niveau de gemme déjà corrigé) sont bien reconnus en mode avancé. Point de vigilance de la section précédente levé, rien à corriger.
- **Revue des 27 clés `client_strings.js` non vérifiées (2026-07-23)** : 2 corrigées via les données déjà extraites, sans transcription manuelle ni capture nécessaire :
  - `FLASK_CHARGES` : dérivé de `ItemDisplayChargesNCharges` (EN "Currently has {0} Charges" -> FR "Contient actuellement {0} charges"). **Confirmé fonctionnel en jeu par Arnaud** (fioles bien reconnues).
  - `HEIST_CONTRACT_TARGET` : dérivé de `ItemDisplayHeistContractObjectiveWithValue` (EN "Heist Target: {0} ({1})" -> FR "Objectif du Casse : {0} ({1})"), même forme que l'anglais. Code corrigé et buildé/linté sans erreur, mais **non vérifiable de façon fiable en jeu** : Arnaud n'est pas certain que même la version anglaise du price-check arrive à estimer un contrat de Casse avec valeur cible. Laissé tel quel (la correction reste objectivement meilleure que l'ancien texte anglais en dur), point abandonné, ne pas y revenir sans raison nouvelle.
  - Couverture `client_strings.js` passée de 101/128 à 103/128.
  - `FRACTURED_PREFIX`/`FRACTURED_SUFFIX` : **bug critique trouvé et corrigé le 2026-07-23**, distinct de la simple revue de clés ci-dessus. Ces deux clés restaient l'ancien texte anglais en dur (`'Fractured Prefix Modifier'`), or elles ne sont comparées QUE dans `advanced-mod-desc.ts` (mode "description de mod avancée", activé chez Arnaud) pour taguer une ligne de mod comme `ModifierType.Fractured`. Résultat : plus aucun objet fracturé n'était reconnu par le price-check depuis l'activation de ce mode. Corrigé en appliquant le même schéma de combinaison déjà utilisé et confirmé pour `CRAFTED_PREFIX`/`CRAFTED_SUFFIX` : `ModDescriptionLineFractured` (EN "Fractured {0}" -> FR "Fissuré {0}") entourant le label Prefix/Suffix déjà confirmé -> FR `"Fissuré Modificateur de préfixe : "` / `"Fissuré Modificateur de suffixe : "`. Couverture `client_strings.js` passée à 105/128. **Confirmé fonctionnel en jeu par Arnaud le 2026-07-23** (objets fracturés ET synthétisés testés, les deux passent).
  - Les 23 autres clés restent non résolues (voir `untranslated-client-strings-fr.report.txt` régénéré) : la plupart nécessitent une vraie capture en jeu ou une extraction de table supplémentaire (`HEIST_BLUEPRINT_GEMS`, `HEIST_TARGET_PRICELESS`), les organes Metamorph (`METAMORPH_BRAIN/EYE/LUNG/HEART/LIVER`) sont **du contenu legacy, plus dans le jeu, non prioritaire** (confirmé par Arnaud), et les mods d'influence (`SHAPER_MODS` etc.) restent un gros morceau à part (extraction `Mods.dat`).
- **Bug systémique trouvé et corrigé le 2026-07-23 : mauvais fichier `StatDescriptions` gagnant sur un texte anglais partagé entre plusieurs fichiers.** Signalé par Arnaud via une ligne de Joyau rare ("10% d'Augmentation de Dégâts (fractured)" sur fissuré, "4% ... (implicit)" sur synthétisé) affichée "Not recognized modifier" alors que la traduction semblait juste. Root cause dans `generate-stats.mjs`/`parse-stat-descriptions.mjs` : `textIndex.get(text)` renvoie TOUS les blocs (tous fichiers confondus) qui partagent le même texte anglais, et le code prenait simplement le premier de la liste, dans l'ordre `fs.readdirSync` (alphabétique) des 13 fichiers. Or plusieurs fichiers définissent un bloc avec un texte anglais identique à `stat_descriptions.txt` (le fichier générique des mods d'objet) mais un texte français légèrement différent, car ce sont en réalité des contextes différents (tooltip de gemme active, tooltip d'Aptitude, tooltip de Monstre, tooltip de Talent passif - jamais du texte copiable d'objet). Exemple concret : `damage_+%` ("{0}% increased Damage") existe à la fois dans `stat_descriptions.txt` (FR "d'Augmentation **de** Dégâts", correct) et dans `active_skill_gem_stat_descriptions.txt` (FR "d'Augmentation **des** Dégâts", pour le tooltip de gemme active) - `active_skill_gem_stat_descriptions.txt` étant alphabétiquement avant `stat_descriptions.txt`, il gagnait systématiquement, cassant TOUTE occurrence de cette stat très commune sur des objets réels (dont l'exemple d'Arnaud).
  - **Recherche exhaustive faite avant de corriger** : script de comparaison croisée de tous les blocs des 13 fichiers, 77 textes anglais identiques trouvés avec un texte français différent selon le fichier (voir liste complète dans l'historique de session si besoin de la revoir).
  - **Correctif** : nouvelle constante `FILE_PRIORITY` dans `generate-stats.mjs`, qui force l'ordre de lecture des fichiers au lieu de l'ordre alphabétique brut. Priorité : `stat_descriptions.txt` (générique, mods d'objet) en premier, puis les fichiers de types d'objets réellement copiables (map/atlas/heist_equipment/tincture/sentinel/necropolis/expedition_relic), et en dernier recours seulement les fichiers de tooltip non copiables (active_skill_gem/gem/skill/passive_skill/monster) - le parser ne traite QUE du texte copié d'un objet, jamais un tooltip de gemme/Aptitude/Talent/Monstre. La fonction lève une erreur explicite si un nouveau fichier apparaît sur le disque sans être dans `FILE_PRIORITY` (évite un oubli silencieux si GGG ajoute un 14ᵉ fichier).
  - **Résultat** : couverture globale inchangée (87.6%, 9017/10299) - c'est normal, il ne s'agit pas de nouvelles traductions mais de corriger LAQUELLE des traductions existantes est utilisée en cas de doublon. 48 lignes de `fr/stats.ndjson` corrigées, dont un bonus trouvé au passage : `#% increased Strength Requirement` avait un texte anglais `"3%"` codé en dur au lieu du placeholder `#` dans `gem_stat_descriptions.txt` (ne matchait donc qu'exactement 3%), maintenant correct via `stat_descriptions.txt`.
  - `.index.bin` régénérés (`npm run make-index-files`). **Confirmé fonctionnel en jeu par Arnaud le 2026-07-23.**
- **Deuxième variante du même bug systémique, trouvée juste après (2026-07-23) : conflit DANS un seul fichier, pas seulement entre fichiers.** Signalé par Arnaud sur une Bague (ligne "#% des Dégâts physiques des Attaques Drainent du Mana", suffixe "de la Soif") non reconnue. Root cause : `stat_descriptions.txt` contient 27 blocs dont l'id commence par `old_do_not_use_` (convention GGG pour un ancien id de stat remplacé, gardé uniquement pour les objets existants roll avant le rework). Pour ce mod précis, `old_do_not_use_mana_leech_from_physical_damage_%` (FR "d'Attaque", singulier, apparaît en premier dans le fichier) partage le même texte anglais que l'id actuel `mana_leech_from_physical_attack_damage_permyriad` (FR "des Attaques", pluriel, confirmé correct) - même bug de "premier candidat gagne" que pour `damage_+%`, mais cette fois les deux blocs sont dans le MÊME fichier, donc le fix `FILE_PRIORITY` ne suffisait pas.
  - **Correctif** : nouvelle fonction `isDeprecatedBlock` dans `parse-stat-descriptions.mjs` (un bloc est déprécié si TOUS ses ids commencent par `old_do_not_use`), et `buildEnglishTextIndex` classe désormais les blocs non-dépréciés avant les dépréciés pour un même texte anglais, au lieu de garder l'ordre brut d'apparition dans le fichier. Un bloc déprécié reste utilisable en dernier recours si aucune autre source n'existe pour ce texte.
  - **Résultat** : couverture inchangée (87.6%, 9017/10299, même raison que le fix précédent), 27 lignes supplémentaires de `fr/stats.ndjson` corrigées. `.index.bin` régénérés. **Confirmé fonctionnel en jeu par Arnaud le 2026-07-23** (Bague "Glyph Nail", suffixe "de la Soif").
  - **À garder en tête pour la suite** : ces deux fixes (priorité de fichier + dépriorisation des ids dépréciés) couvrent les schémas de conflit trouvés jusqu'ici, mais rien ne garantit qu'il n'existe pas un 3ᵉ type de conflit non encore rencontré. Si un nouveau cas "traduction plausible mais objet non reconnu" remonte, vérifier d'abord si le texte anglais concerné existe en double (même technique que cette session : comparer tous les blocs `raw/stat-descriptions/*.txt` partageant le même texte anglais) avant de chercher une cause plus compliquée.
- **3ᵉ variante du même problème racine, trouvée et partiellement corrigée le 2026-07-23 : un même id de stat défini DEUX FOIS dans le même fichier, avec un français différent.** Contrairement aux deux bugs précédents, il n'existe aucun signal de nommage (pas de préfixe `old_do_not_use`, pas de fichier différent) pour savoir lequel des deux blocs est le bon - vérification au cas par cas obligatoire.
  - **Cas confirmé et corrigé** : `cannot_be_poisoned` (stat "Cannot be Poisoned", actif, avec id fracturable) apparaît deux fois dans `stat_descriptions.txt` : "Immunité à l'Empoisonnement" (1ᵉʳ bloc, utilisé par erreur jusqu'ici) et "Vous ne pouvez pas être Empoisonné" (2ᵉ bloc). Arnaud a trouvé un objet réel portant ce mod ("La Lame vivante", épée à deux mains unique) et confirmé que le texte en jeu est bien "Vous ne pouvez pas être Empoisonné" - recoupé avec poewiki.net/wiki/Poison qui indique que cette formulation est propre à cet unique. Corrigé via une nouvelle table `CONFIRMED_TEXT_OVERRIDES` dans `generate-stats.mjs` (même principe que `TIMELESS_JEWEL_HISTORIC_ADVANCED` : un override ciblé, ajouté uniquement après vérification réelle, jamais par déduction). **Confirmé fonctionnel en jeu par Arnaud.**
  - **Important : "le dernier bloc gagne" n'est PAS une règle générale fiable.** En cherchant tous les ids dupliqués dans un même fichier (16 trouvés au total, tous fichiers confondus), certains cas semblent au contraire indiquer que le PREMIER bloc est le bon (ex. `base_cannot_gain_bleeding` : le 1ᵉʳ dit "ne peut **pas** vous être infligé", le 2ᵉ dit "ne peut vous être infligé" - une négation manquante qui sent la coquille GGG dans le second). Chaque cas doit être vérifié individuellement avant correction, jamais deviné en bloc.
  - **Liste des ids dupliqués restants, non vérifiés, à traiter au cas par cas si un bug remonte dessus** (texte FR différent entre les deux blocs, need vérification en jeu) :
    - `stat_descriptions.txt` : `base_aura_area_of_effect_+%`, `map_unstable_breach_spawns_boss` (texte totalement différent entre les 2 blocs, à investiguer en priorité si ça remonte), `map_player_quantity_does_not_apply` (1ᵉʳ bloc a un texte FR vide - probablement le 2ᵉ est le bon par défaut, mais pas vérifié en jeu), `offering_spells_effect_+%`, `base_cannot_gain_bleeding`, `all_damage_can_ignite`, `all_damage_can_freeze`, `all_damage_can_shock`
    - `active_skill_gem_stat_descriptions.txt` : `base_aura_area_of_effect_+%` (contexte tooltip de gemme, priorité basse de toute façon)
    - `atlas_stat_descriptions.txt`, `gem_stat_descriptions.txt` : quelques autres cas mineurs, détail dans l'historique de session si besoin
  - **31 autres cas trouvés au total (ids DIFFÉRENTS partageant le même texte anglais dans un même fichier, hors doublons d'id)** : la plupart sont probablement des mods réellement distincts qui rendent le même texte anglais par coïncidence - pas un bug de pipeline à proprement parler, juste une ambiguïté de la donnée source qui ne peut être tranchée que par la vraie donnée en jeu. Pas traité aujourd'hui faute de signal clair, à traiter en réactif si un cas remonte.
