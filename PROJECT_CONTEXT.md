...
- **Dernière étape terminée** : étape 3 (parser d'objets fr), testée en jeu, committée sur `master` en local (commit `266ccdb`, **pas encore pushée**). Étape 2 (localisation UI complète) committée et pushée sur `origin/master`, commit `12babfa`. **Le correctif Timeless Jewel du 2026-07-22 (`fr/stats.ndjson` + `generate-stats.mjs`) n'est pas encore committé.**
- **Prochaine tâche (à la reprise)** :
  1. **Committer le correctif Timeless Jewel** (`fr/stats.ndjson` régénéré + `generate-stats.mjs` avec `TIMELESS_JEWEL_HISTORIC_ADVANCED`) — en attente, pas fait automatiquement.
  2. **Investiguer la Tincture** (mentionnée depuis la session du 21/07, jamais testée) — probablement un bug différent du Timeless Jewel puisque celui-ci est maintenant résolu et avait une cause bien spécifique (texte avancé non dérivable). À vérifier au cas par cas plutôt que de supposer une cause commune.
  3. Décider de la proposition de PR au mainteneur original (étape 5), ou continuer à affiner la couverture (stats Heist/Atlas restantes, section 9.3).
  4. Termes de lore de l'étape 1 (app_i18n.json, liste en section 4) toujours pas vérifiés avec le client PoE FR réel — non bloquant, à faire à l'occasion.
